<template>
  <div class="HTable" style="margin-top:10px">
    <el-table :key="JSON.stringify(checkedCol)" :data="tableData" v-bind="$attrs" width="100%" v-on="$listeners"
              @selection-change="handleSelectionChange">
      <el-table-column v-for="(item, index) in checkedCol" :key="item.prop" :align="item.align ? item.align : 'center'"
                       :column-key="item.prop"
                       :index="index" :selectable="checkSelectable" v-bind="item">
...
export default {
  ...
  methods: {
    checkSelectable(row) {
      if (!this.$attrs.checkSelectable) {
        return true
      }
      var that = this
      return new Function('that', 'row', this.$attrs.checkSelectable)(that, row)
      // let result = eval(this.$attrs.checkSelectable)
      // return result
    },
