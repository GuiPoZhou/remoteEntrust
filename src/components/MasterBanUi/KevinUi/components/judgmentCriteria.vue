<template>
  <div class="groupAdd">
    <ml-dialog ref="mlDialog" :dia-log-show="addShow" dia-log-title="指定判定标准" dia-log-width="60%" @close="close">
      <template slot="bologbody">
        ...
        <el-row>
          <el-col :span="9" style="padding-right: 10px">
            ...
            <el-table ref="categoryList" :data="categoryList" :row-key="getRowKey"
                      :tree-props="{ children: 'itemLimitList' }" border
          ...
          <el-col :span="15">
            ...
            <el-table ref="multipleTableDevice" :data="tableList" :row-class-name="tableRowClassName" border
              ...
              <el-table-column align="center" label="检测项目标识" prop="itemId" show-overflow-tooltip/>
...
export default {
  ...
  data() {
    return {
      ...
      callbackEvents: '',//回调函数脚本
    }
  },
  methods: {
    init(data, params, callbackEvents) {
      this.callbackEvents = callbackEvents
      console.log('callbackEvents', callbackEvents)
      data = JSON.parse(JSON.stringify(data))
      data.forEach(item => {
        if (!item.itemLimitList) {
          item.itemLimitList = []
        }
      })
      this.addShow = true
      this.tableList = data
      this.newtableList = data
    ...
    getChildData(arr) {
      arr.map((item) => {
        ...
        if (item.itemLimitList.length != 0) {
          this.getChildData(item.itemLimitList)
        }
      })
    },
    // 选择执行标准
    ...
    //         items.productionName = itembiaozhun.standardTypeName
    ...
    //     for (var x in newarr) {
    //       if (newarr[x].itemName == this.projectList[i].itemName) {
    //         newList.push(newarr[x]);
    //       }
    //     }
    //     if (newarr) {
    //       this.$set(this.projectList[i], 'itemLimitList', newList)
    //     }
    //     var newList1 = new Array;
    //     var objs = JSON.parse(localStorage.getItem('ItemLimits'))
    //     for (var y in objs) {
    //       if (objs[y].itemName == this.projectList[i].itemName) {
    ...
    //     (a) => !num.some((b) => a.itemName === b.itemName)
    //   )
    //   var arrs = []
    ...
    //     message += '<p>' + newData[i].itemName + '检测项目不存在判定限值</p>'
    //   }
    ...
    async e_confrim() {
      ...
      var newarr = new Array;
      for (var i in list) {
        var index = arr.indexOf(list[i].decideStandardId + '_' + list[i].itemId);
        if (index < 0) {
          arr.push(list[i].decideStandardId + '_' + list[i].itemId);
          newarr.push(list[i]);
        } else {
          if (newarr[index].limitDesc && list[i].limitDesc) {
            newarr[index].limitDesc = newarr[index].limitDesc + ',' + list[i].limitDesc
      ...
      for (var i in this.projectList) {
        var newList = new Array;
        for (var x in newarr) {
          if (newarr[x].itemId == this.projectList[i].itemId) {
            newList.push(newarr[x]);
          }
        }
        if (newarr) {
        ...
        var objs = JSON.parse(localStorage.getItem('ItemLimits'))
        for (var y in objs) {
          if (objs[y].itemId == this.projectList[i].itemId) {
            newList1.push(objs[y]);
          }
        }
        if (objs) {
      ...
      var num = this.fn2(proList.data)
      let newData = this.projectList.filter(
          (a) => !num.some((b) => a.itemId == b.itemId)
      )
      var arrs = []
      this.projectList.map(items => {
        num.map(item => {
      ...
      var message = '<p>共指定检测项目' + this.projectList.length + '个，' +
          ...
          (this.projectList.length - arrs.length) + '个未成功。</p>'
      for (var i in newData) {
        message += '<p>' + newData[i].itemId + '检测项目不存在判定限值</p>'
      }
    ...
    async close() {
      // var objs = JSON.parse(localStorage.getItem('ItemLimits'))
      // var proList = await this.$net(`/decideStandard/v1/standard/${this.ids.join(',')}/getItemLimits`, 'get')
      // for (var i in this.newtableList) {
      //   var newList1 = new Array;
      //   for (var x in objs) {
      //     if (objs[x].itemName == this.newtableList[i].itemName) {
      //       newList1.push(objs[x]);
      //     }
      //   }
      //   this.$set(this.newtableList[i], 'itemLimitList', newList1)
      // }
      // this.$emit('close', this.tableList, this.newtableList)
      localStorage.removeItem('ItemLimits')
      this.$emit('close', this.tableList, this.callbackEvents)
      this.projectList = []
      this.tableList = []
      this.biaozhunList = []
    }
