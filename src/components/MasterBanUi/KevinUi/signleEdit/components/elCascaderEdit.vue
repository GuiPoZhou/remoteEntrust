    ...
    <el-row style="padding: 2rem;">
      <el-col :span="10">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="选择事件脚本" prop="events.change">
              <el-input v-model="signleParams.events.change" readonly>
                <el-button slot="append" size="small" @click="e_editChangeScript">编辑</el-button>
              </el-input>
            </el-form-item>
          </el-col>
          <el-form-item>
...
export default {
  ...
  methods: {
    e_editChangeScript() {
      this.$refs.KevinEditor.changeEditor({value: this.signleParams.events.change})
    },
    handleEditorInput(code) {
      this.$set(this.signleParams.events, 'change', this.formatCode(code))
      this.$forceUpdate()
    },
    e_save() {
    ...
    appendParams(params) {
      ...
      if (!this.signleParams.events || !this.signleParams.events.change) {
